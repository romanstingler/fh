...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Script for finding the fundamental matrix of a system.')
    parser.add_argument('-m','--matrix', help='Matrix',required=True)
    parser.add_argument('-x','--initialvalue', help='Initial value',required=False)
    parser.add_argument('-t0','--initialt', help='Initital value t',required=False)
    ...
    fundSystem=CalcFundSys(a, genVectors['genBasis'],genVectors['lam'],genVectors['deg'])
