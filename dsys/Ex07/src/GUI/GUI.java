...
public class GUI extends javax.swing.JFrame {
	...
	private Vector<JComponent> componentsToEnable; // components which should be
													// enabled when logged on
	private Vector<JComponent> componentsToDisable; // components which should
													// be disabled when logged
													// on
		...
		layout.setHorizontalGroup(layout
				...
				.addGroup(
						...
						layout.createSequentialGroup()
								...
								.addComponent(jLabel2).addGap(268, 268, 268))
				.addGroup(
						layout.createSequentialGroup()
								.addGroup(
								...
								.addGroup(
										layout.createParallelGroup(
												...
												.addComponent(
														...
														199, Short.MAX_VALUE))
								.addGap(51, 51, 51))
				.addComponent(tstate, javax.swing.GroupLayout.DEFAULT_SIZE,
						809, Short.MAX_VALUE));
	...
	private void bLogoutActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_bLogoutActionPerformed
		...
		if (client.logout()) {
			...
			JOptionPane.showMessageDialog(this,
					"Sorry, cannot log out! Please try again!");
	...
	private void bLoginActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_bLoginActionPerformed
		...
		try {
			clientiimpl = new ClientIImpl(tshareddir.getText(),
					tusername.getText());
			client.setClientIImpl(clientiimpl);

			if (!client.login(tusername.getText(),
					new String(tpassword.getPassword()), tserver.getText())) {
				JOptionPane.showMessageDialog(this,
						"Sorry, cannot log on! Please try again!");
				this.setLoggedOn(false);
			} else {
				this.setLoggedOn(true);
			}
	...
	private void binfoActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_binfoActionPerformed
		...
		JOptionPane
				.showMessageDialog(
						this,
						"user: " + gfi.getUsername() + ",file: "
								+ gfi.getFilename() + ", bytes: "
								+ gfi.getLength());
	...
	private void bdownloadActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_bdownloadActionPerformed
		...
		// Get GlobalFileInfo from the selection in the GUI
		GlobalFileInfo gfi = (GlobalFileInfo) lresults.getSelectedValue();

		// Get filesize in bytes and peer that has the file
		...
		try {
			// Call remote method getFile on peer - returns a RemoteFISI object
			// (serializable object with method to read byte from file)
			RemoteFISI fis = peer.getFile(gfi.getAbsFilepath());

			if (fis != null) {
				FileOutputStream fos = new FileOutputStream(
						tshareddir.getText() + DELIM + gfi.getFilename());

				pdownloadprogress.setMinimum(0);
				pdownloadprogress.setMaximum((int) len);

				// read the remote file bytewise and write it to a file with
				// same name in the own shared directory
		...
		try {
			// open the downloaded file with the specified application
			String command = topenCommand.getText() + " "
					+ tshareddir.getText() + DELIM
					+ lresults.getSelectedValue().getFilename();
