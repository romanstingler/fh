...
public class Ex0501 {
	// 5.1
	// The happened-before relation is a relation between the result of two
	// events.
	...
	// that Lamport timestamps does not guarantee if e1 has a smaller time stamp
	// than e2 that it happened before
	// because concurrent events are ordered arbitrarily.
	// To detect causality violation, a VT is needed with must contain
	// information about the other processors.

	// 5.4
	// (A)
	// The two vectors are compared component-by-component e.g. [1,2,3] and
	// [2,4,3] so (1 with 2 , 2 with 4 and 3 with 3).
	//
	// (B)
	// The vector components are set to the higher value, additionally the local
	// time is increased by 1.
	//
	// (C)
	// If two events are concurrent no path between e1 and e2 are allowed in the
